...
const CATEGORIES: Array<{ value: DeliveryExceptionCategory; label: string }> = [
  { value: "shortage", label: "Shortage" }, { value: "damage", label: "Damage" }, { value: "quantity_discrepancy", label: "Quantity discrepancy" },
  { value: "wrong_material", label: "Wrong material" }, { value: "refused", label: "Refused" }, { value: "site", label: "Site" }, { value: "vehicle", label: "Vehicle" }, { value: "other", label: "Other" },
];

export function DeliveryExceptionPanel({ orgId, userId, note, delivery, onSaved, onError }: {
  ...
  const reload = async () => {
    const [updatedNote, updatedDelivery] = await Promise.all([deliveryNotesRepo.getById(orgId, note.id), delivery ? deliveriesRepo.getById(orgId, delivery.id) : Promise.resolve(null)]);
  ...
  const createException = async () => {
    ...
    try {
      const payload: Omit<DeliveryException, keyof BaseRecord> = {
        deliveryNoteId: note.id, deliveryId: delivery.id, category, description: description.trim(), reportedBy: userId, reportedAt: Timestamp.now(),
        status: "open", evidenceRefs: [], resolutionNotes: "", resolvedBy: null, resolvedAt: null,
      };
      ...
      setDescription(""); await reload();
    } catch (err) { onError(err instanceof Error ? err.message : "Failed to save the delivery exception."); }
  ...
  return <section className="rounded-lg border border-slate-800 bg-slate-950/60 p-4">
    ...
    <div className="mt-3 grid gap-2 md:grid-cols-[180px_1fr_auto]">
      <select className="input" value={category} onChange={(event) => setCategory(event.target.value as DeliveryExceptionCategory)} disabled={busy}>{CATEGORIES.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select>
