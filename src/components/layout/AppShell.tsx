...
const NAV_ITEMS: NavItem[] = [
  ...
  { label: "Fleet & Live Map", href: (id) => `/${id}/trucks`, icon: "▣", roles: ["owner", "operations_manager", "dispatcher", "fleet_manager", "viewer"] },
  ...
  { label: "Delivery Notes & POs", href: (id) => `/${id}/deliveries`, icon: "✓", roles: ["owner", "operations_manager", "dispatcher", "fleet_manager", "viewer"] },
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  const { activeOrg, activeMembership, organizations, memberships, setActiveOrgId } = useWorkspace(); const { user, signOut } = useAuth(); const pathname = usePathname(); const router = useRouter(); const [mobileOpen, setMobileOpen] = useState(false);
  ...
  const switchWorkspace = (orgId: string) => { if (orgId === activeOrg.id) return; const membership = memberships.find((m) => m.orgId === orgId); setActiveOrgId(orgId); router.replace(`/${orgId}/${membership?.role === "driver" ? "my-trip" : "control-tower"}`); };
