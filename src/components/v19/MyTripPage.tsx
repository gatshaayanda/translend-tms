...
export default function MyTripPage() {
  ...
  const advance=async()=>{
    ...
    try{
      const payload={orgId:activeOrg.id,tripId:trip.id,status:next,idempotencyKey:newDriverActionId()};
      if(!navigator.onLine){
  ...
  return <div className="space-y-6"><header className="page-header"><div><h1 className="page-title">My Trip</h1><p className="page-subtitle">Your next action, delivery, vehicle condition and tools in one driver-first view.</p></div><span className="badge green">{LABELS[trip.status]}</span></header>
    {!online&&<div className="notice" style={{borderColor:"#E6B94A",background:"#FFF8E1"}}><strong>Offline.</strong> Driver trip changes, delivery actions and vehicle inspections are saved locally and will sync when the connection returns.</div>}{message&&<div className="notice blue">{message}</div>}
