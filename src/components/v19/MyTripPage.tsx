...
export default function MyTripPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setDriver(linked);
      setTrips(linked ? tripList.filter((trip) => trip.driverId === linked.id && trip.status !== "completed") : []);
    }).catch((error) => setMessage(error instanceof Error ? error.message : "Unable to load your trip."));
    return () => { alive = false; };
  }, [activeOrg, user]);
  ...
  const advance = async () => {
    if (!activeOrg || !user || !trip) return;
    const next = FLOW[FLOW.indexOf(trip.status) + 1];
    if (!next) return;
    setMessage(null);
    try {
      const token = await user.getIdToken();
      const response = await fetch("/api/driver/trip-status", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ orgId: activeOrg.id, tripId: trip.id, status: next }) });
  ...
  if (!driver) return <div className="space-y-6"><header className="page-header"><div><h1 className="page-title">My Trip</h1><p className="page-subtitle">Your driver workspace on Translend.</p></div><span className="badge">DRIVER APP</span></header><div className="notice blue">Your Google account is signed in, but it is not linked to a Translend driver record yet. Ask the fleet manager to link your driver profile.</div></div>;
  if (!trip) return <div className="space-y-6"><header className="page-header"><div><h1 className="page-title">My Trip</h1><p className="page-subtitle">Your next assigned movement appears here.</p></div><span className="badge green">READY</span></header><div className="panel"><h2 className="section-title">No active trip</h2><p className="section-sub">You are linked as {driver.fullName}. When dispatch assigns you a trip, it will appear here.</p></div></div>;

  const next = FLOW[FLOW.indexOf(trip.status) + 1];
  return <div className="space-y-6">
    <header className="page-header"><div><h1 className="page-title">My Trip</h1><p className="page-subtitle">A focused mobile workflow for drivers: trip state, location and delivery handoff.</p></div><span className="badge green">{LABELS[trip.status]}</span></header>
    ...
    <div className="notice blue">Install Translend on this phone for the best driver workflow. Location still requires an explicit permission and the app must remain active for browser GPS capture.</div>
  </div>;
}
