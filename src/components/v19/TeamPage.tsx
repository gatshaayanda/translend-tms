...
export default function TeamPage() {
  ...
  const load = async () => {
    ...
    } catch (err) { setError(err instanceof Error ? err.message : "Unable to load the team. Try again or report the issue to the Translend developer/workspace administrator."); }
  ...
  const invite = async (event: FormEvent) => {
    ...
    } catch (err) { setError(err instanceof Error ? err.message : "Invitation failed. Report the issue to the Translend developer/workspace administrator if it persists."); }
  ...
  return <div className="space-y-6">
    ...
    <section className="panel"><div className="section-header"><div><h2 className="section-title">Invite a person</h2><p className="section-sub">No Firebase UID is required up front. The invitation is pending until the invited email signs in.</p></div></div>
      <form onSubmit={invite} className="form-grid"><label className="form-group"><span className="field-label">Email</span><input className="form-input" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="person@company.com" required /></label><label className="form-group"><span className="field-label">Role</span><select className="form-select" value={role} onChange={(e) => setRole(e.target.value as Exclude<OrgRole, "owner">)}>{INVITABLE_ROLES.map((item) => <option key={item} value={item}>{ROLE_LABELS[item]}</option>)}</select></label><div className="form-actions" style={{ alignItems: "end" }}><button className="btn-primary" disabled={saving}>{saving ? "Creating invitation…" : "Invite person"}</button></div></form>
    </section>
