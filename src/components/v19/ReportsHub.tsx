...
export function ReportsHub({ orgId }: { orgId: string }) {
  ...
  const podRows = useMemo(() => data.deliveries.map((delivery) => {
    ...
    return {
      delivery: delivery.deliveryNoteId ?? delivery.id.slice(0, 8),
      job: trip?.jobNumber ?? delivery.jobId,
      truck: trip?.truckRegistration ?? "—",
      status: delivery.status,
      podState: delivery.podState ?? "not_started",
      required: required.length,
      approvedRequired: approvedRequired.length,
      rejected: rejected.length,
      readiness: required.length > 0 && approvedRequired.length >= required.length ? "ready" : delivery.status === "exception" ? "exception" : "needs evidence",
    };
  }), [data]);
