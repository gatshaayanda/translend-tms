...
function latestByTruck(events: TruckLocationEvent[]) {
  ...
  for (const event of events) {
    ...
    if (!previous || (event.capturedAt?.toMillis?.() ?? 0) >= (previous.capturedAt?.toMillis?.() ?? 0)) latest.set(event.truckId, event);
  }
  return [...latest.values()];
}
function eventIsStale(event: TruckLocationEvent) { const captured = event.capturedAt?.toMillis?.() ?? 0; return !captured || Date.now() - captured > STALE_AFTER_MS; }
function statusFor(event: TruckLocationEvent) { if (eventIsStale(event)) return "stale" as const; return event.status === "moving" ? "moving" as const : "idle" as const; }

export function FleetLiveMap({ events, trucks }: { events: TruckLocationEvent[]; trucks: Array<{ id: string; registration?: string; make?: string; model?: string }> }) {
  ...
  const rows = useMemo(() => latest.map(event => ({ event, truck: truckById.get(event.truckId) })).filter(({ event }) => {
    const status = statusFor(event);
    return filter === "all" || filter === status || (filter === "alert" && status === "stale");
  }).sort((a, b) => (b.event.capturedAt?.toMillis?.() ?? 0) - (a.event.capturedAt?.toMillis?.() ?? 0)), [latest, truckById, filter]);
  ...
  useEffect(() => {
    ...
    const initialise = () => {
      ...
      mapRef.current = new window.google.maps.Map(mapElement.current, { center: DEFAULT_CENTER, zoom: 10, mapTypeControl: false, streetViewControl: false, fullscreenControl: true, clickableIcons: false });
      setMapReady(true);
    };
    if (window.google?.maps) { initialise(); return () => { cancelled = true; }; }
    ...
    if (existing) {
      ...
      return () => { cancelled = true; existing.removeEventListener("load", initialise); };
    }
    const script = document.createElement("script");
    script.src = `https://maps.googleapis.com/maps/api/js?key=${encodeURIComponent(apiKey)}&loading=async`;
    script.async = true; script.defer = true; script.dataset.translendGoogleMaps = "true";
    ...
    return () => { cancelled = true; script.removeEventListener("load", initialise); };
  }, [apiKey]);
  ...
  useEffect(() => {
    ...
    for (const [truckId, marker] of markersRef.current) {
      if (!activeIds.has(truckId)) { marker.setMap(null); markersRef.current.delete(truckId); }
    }
    for (const { event, truck } of rows) {
      const status = statusFor(event), label = truck?.registration ?? event.truckId;
      let marker = markersRef.current.get(event.truckId);
      if (!marker) {
        marker = new window.google.maps.Marker({ map, title: label });
  ...
  const focus = (event: TruckLocationEvent) => {
    ...
    if (mapRef.current) { mapRef.current.panTo({ lat: event.latitude, lng: event.longitude }); mapRef.current.setZoom(14); }
  };

  return <section className="panel">
    <div className="section-header"><div><h2 className="section-title">Live fleet map</h2><p className="section-sub">Real driver GPS or telematics positions only. Stale positions are flagged instead of being presented as live.</p></div><div className="form-row-actions">{(["all", "moving", "idle", "alert"] as MapFilter[]).map(value => <button key={value} className={filter === value ? "btn-primary" : "btn-secondary"} onClick={() => setFilter(value)}>{value === "all" ? "All" : value === "alert" ? "Alerts" : value[0].toUpperCase() + value.slice(1)}</button>)}</div></div>
