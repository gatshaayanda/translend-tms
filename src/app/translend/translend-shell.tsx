...
const collectionByModule: Partial<Record<ModuleKey, BusinessCollection>> = {
  Jobs: 'jobs', Dispatch: 'dispatches', Trips: 'trips', Routes: 'routes', Deliveries: 'deliveries', POD: 'pods', Exceptions: 'exceptions', Maintenance: 'maintenance', Inspections: 'inspections', Compliance: 'compliance', Tyres: 'tyres', Fuel: 'fuel', Expenses: 'expenses', Invoices: 'invoices', Receipts: 'receipts', Statements: 'statements', 'Rate Cards': 'rateCards',
}
...
export default function TranslendShell() {
  ...
  return <div className={styles.app}>
    <aside className={`${styles.sidebar} ${mobileOpen ? styles.sidebarOpen : ''}`}>
      ...
      <nav className={styles.nav}>{navigation.map(section => <div key={section.label} className={styles.navSection}><div className={styles.sectionLabel}>{section.label}</div>{section.items.map(({ label, icon: Icon }) => <button key={label} className={`${styles.navItem} ${active === label ? styles.navItemActive : ''}`} onClick={() => select(label)}><Icon size={17} /><span>{label}</span></button>)}</div>)}</nav>
