...
export default function TranslendShell() {
  ...
  useEffect(() => onAuthStateChanged(translendAuth, async (nextUser) => {
    setUser(nextUser ? { displayName: nextUser.displayName, email: nextUser.email } : null)
  ...
  return <div className={styles.app}>
    ...
    <div className={styles.main}>
      ...
      <main className={styles.content}>
        {active === 'Dashboard' ? <Dashboard jobs={jobs} trucks={trucks} drivers={drivers} invoices={invoices} attention={attention} onOpen={select} organization={organization} /> : <ModuleView module={active} rows={activeRows} onAdd={() => setShowForm(true)} onRemove={removeRecord} />}
        {showForm && active !== 'Dashboard' && <RecordForm module={active} onCancel={() => setShowForm(false)} onSave={addRecord} />}
        <div className={styles.footerNote}><span>Translend TMS · company workspace</span><span>Business records are ready to connect to this private workspace.</span></div>
      </main>
    </div>
  </div>
}
...
function ModuleView({ module, rows, onAdd, onRemove }: { module: ModuleKey; rows: RecordItem[]; onAdd: () => void; onRemove: (index: number) => void }) {
  ...
  return <section className={styles.panel}><div className={styles.panelHeader}><div><span className={styles.panelEyebrow}>Company workspace</span><h2>{module}</h2><p style={{ marginTop: 5, color: '#718188', fontSize: 11 }}>{description}</p></div><button className={styles.primaryButton} onClick={onAdd} title="Record persistence is connected in the next data-layer step"><Plus size={14} /> Add record</button></div><div className={styles.tableWrap}>{rows.length ? <table className={styles.table}><thead><tr>{fields.map((field) => <th key={field}>{field}</th>)}<th>Action</th></tr></thead><tbody>{rows.map((row, index) => <tr key={`${module}-${index}`}><td><strong>{row[fields[0]] || '—'}</strong></td>{fields.slice(1).map((field) => <td key={field}>{row[field] || '—'}</td>)}<td><button className={styles.textButton} onClick={() => onRemove(index)}>Delete</button></td></tr>)}</tbody></table> : <EmptyState text={`No ${module.toLowerCase()} records yet. This area is clean for your company's first records.`} />}</div></section>
}
