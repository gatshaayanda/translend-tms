...
const collectionByModule: Partial<Record<ModuleKey, BusinessCollection>> = { Trucks: 'trucks', Drivers: 'drivers' }
...
function Dashboard({ organization, onOpen }: { organization: TranslendOrganization | null; onOpen: (module: ModuleKey) => void }) {
  return <><section className={styles.pageHeader}><div><span className={styles.eyebrow}>Control Tower</span><h1>{organization?.name || 'Your company'}.</h1><p>Your private transport workspace is ready. Fleet master data is now connected to Firestore; operational records will build on it next.</p></div><div className={styles.headerActions}><button className={styles.secondaryButton} onClick={() => onOpen('Drivers')}>Drivers</button><button className={styles.primaryButton} onClick={() => onOpen('Trucks')}>Trucks</button></div></section><section className={styles.kpiGrid}><Kpi label="Jobs" value="—" note="Operational core next" /><Kpi label="Trucks" value="Connected" note="Firestore-backed" /><Kpi label="Drivers" value="Connected" note="Firestore-backed" /><Kpi label="Customers" value="Connected" note="Firestore-backed" /></section><section className={styles.dashboardGrid}><article className={styles.panel}><div className={styles.panelHeader}><div><span className={styles.panelEyebrow}>Operational spine</span><h2>Customer → Job → Trip → Delivery</h2></div></div><div className={styles.movementRows}><Movement label="Customers" value="Live" /><Movement label="Fleet" value="Live" /><Movement label="Operations" value="Next" /><Movement label="Finance" value="Later" /></div></article><article className={styles.panel}><div className={styles.panelHeader}><div><span className={styles.panelEyebrow}>Workspace</span><h2>Clean company data</h2></div></div><div className={styles.tableWrap}><EmptyState text="No demo company records are being inserted. Your organization starts with its own data." /></div></article></section></>
}
