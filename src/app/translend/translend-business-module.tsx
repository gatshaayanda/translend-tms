...
export default function TranslendBusinessModule({ collectionName, organizationId, userId }: Props) {
  ...
  async function refresh() { setLoading(true); setError(''); try { setRows(await getBusinessRecords(organizationId, collectionName)) } catch (err) { setError(err instanceof Error ? err.message : `Unable to load ${config.title.toLowerCase()}.`) } finally { setLoading(false) } }
  ...
  const singular = collectionName === 'trucks' ? 'truck' : 'driver'
  return <section className={styles.panel}>
    <div className={styles.panelHeader}><div><span className={styles.panelEyebrow}>Fleet master data</span><h2>{config.title}</h2><p style={{ marginTop: 5, color: '#718188', fontSize: 11 }}>{config.description}</p></div><button className={styles.primaryButton} onClick={startCreate}>+ Add {singular}</button></div>
