...
export async function POST(request: Request) {
  try {
    ...
    if (!auth.email || auth.email_verified === false) {
      return NextResponse.json({ claimed: false, reason: "A verified Google email is required." }, { status: 403 });
    }
    const email = auth.email.trim().toLowerCase();
    const snap = await adminDb.collection("workspaceInvites").where("email", "==", email).get();
    const now = Date.now();
    const candidate = snap.docs
      .filter((doc) => doc.data().status === "pending" && doc.data().expiresAt?.toMillis?.() > now)
    ...
    const orgRef = adminDb.doc(`organizations/${invite.orgId}`);
    const memberRef = adminDb.doc(`organizations/${invite.orgId}/members/${auth.uid}`);
