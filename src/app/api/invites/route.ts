...
async function requireManager(uid: string, orgId: string) {
  const member = await adminDb.doc(`organizations/${orgId}/members/${uid}`).get();
...
export async function GET(request: Request) {
  try {
    ...
    const snap = await adminDb.collection("workspaceInvites").where("orgId", "==", orgId).get();
    const now = Date.now();
...
export async function POST(request: Request) {
  try {
    ...
    const org = await adminDb.doc(`organizations/${orgId}`).get();
    if (!org.exists) return NextResponse.json({ error: "Workspace not found." }, { status: 404 });
    const existingMember = await adminDb.collection(`organizations/${orgId}/members`).where("email", "==", email).limit(1).get();
    ...
    const pending = await adminDb.collection("workspaceInvites").where("orgId", "==", orgId).get();
