...
export async function POST(request: Request) {
  try {
    ...
    const orgId = String(body.orgId ?? ""); const tripId = String(body.tripId ?? "");
    const requestedStatus = String(body.status ?? "") as TripStatus; const idempotencyKey = String(body.idempotencyKey ?? "").trim();
    if (!orgId || !tripId || !DRIVER_STATUS_FLOW.includes(requestedStatus) || !idempotencyKey || idempotencyKey.length > 160) throw new ApiError(400, "Trip, valid status and idempotency key are required.");
    ...
    const tripRef = db.doc(`organizations/${orgId}/trips/${tripId}`); const receiptRef = db.doc(`organizations/${orgId}/mutationReceipts/${idempotencyKey}`);
    if ((await receiptRef.get()).exists) return NextResponse.json({ ok: true, status: requestedStatus, duplicate: true });
    ...
    const current = trip.data()?.status as TripStatus; const currentIndex = DRIVER_STATUS_FLOW.indexOf(current); const nextIndex = DRIVER_STATUS_FLOW.indexOf(requestedStatus);
    ...
    const result = await db.runTransaction(async (tx) => {
      ...
      if (requestedStatus === "completed") {
        const data = latest.data()!; const now = FieldValue.serverTimestamp();
