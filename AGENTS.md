...
Driver My Trip supports `planned → en_route_pickup → loading → in_transit → unloading → completed`, assigned-driver filtering, secure server-side status changes, delivery arrival/departure, receiver acknowledgement, delivery exceptions, POD/evidence handoff, inspections, defects, defect→work-order handoff, maintenance/tyre visibility, browser GPS capture and offline awareness. Driver writes are server/role scoped. Driver trip-status transitions now commit the Trip state, completion release of linked Job/Truck/Driver, and immutable audit event in one Firestore transaction with concurrency revalidation. Driver vehicle inspections and defect→work-order creation are also transactional with audit coverage, preventing partial inspection/work-order workflows.
...
Existing invitation lifecycle remains server-controlled. Added secure server member actions for role changes, suspension/restoration, with owner protection, operations-manager limits, audit events and in-app notifications. Controlled owner transfer now runs as one Firestore transaction: current owner transfers ownership to an active member, becomes Operations Manager, organization `ownerUid` is updated, and the ownership audit is committed with the membership/org changes. Ordinary member role/suspension/restore mutations now use the same transaction boundary with atomic audit writes. Team UI exposes member access controls and owner transfer. Firestore membership rules are hardened so clients cannot self-edit their membership documents; membership mutations remain server-controlled.
