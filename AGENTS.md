...
## NEXT: PATCH 2 — REAL DELIVERY WORKFLOW

The next build package is to connect the real operational delivery chain.
...
The goal is not merely to create pages.

The goal is to create a connected operational workflow using the existing
Translend architecture and data model.

Before building:

1. Inspect the existing Job implementation.
2. Inspect the existing Trip implementation.
3. Inspect the existing Delivery implementation.
4. Inspect the current Firestore collections and types.
5. Inspect the UploadThing infrastructure committed in ff313d4.
6. Identify what already works.
7. Extend the existing system rather than replacing it.
...
Leave the repository in a better, verified, committed state for the next AI.
